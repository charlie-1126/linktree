...
export function CodingStats() {
    ...
    useEffect(() => {
        // solved.ac 캐시 확인 및 로드
        const loadSolvedac = () => {
            try {
                const cached = localStorage.getItem(SOLVEDAC_CACHE_KEY);
                if (cached) {
                ...
                console.error("Failed to load cached solved.ac data:", e);
            }
            return false;
        };
        ...
        const loadAtcoder = () => {
            try {
                ...
                console.error("Failed to load cached AtCoder data:", e);
            }
            return false;
        };

        if (loadAtcoder()) return;
        ...
        <div className="space-y-4 sm:space-y-5">
            ...
            <div className="bg-[#1a1a1a] rounded-2xl p-4 sm:p-6">
                <div className="flex items-center gap-3 mb-4 sm:mb-6">
                    ...
                    <h2 className="text-white text-lg sm:text-xl font-semibold">solved.ac</h2>
                </div>
                ...
                <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3 sm:gap-4 mb-4 sm:mb-5">
                    ...
                    <div className="flex-1 grid grid-cols-2 gap-2 sm:gap-3">
                        <div className="bg-[#0f0f0f] rounded-xl p-3">
                            <div className="text-gray-400 text-xs font-medium mb-1">Rating</div>
                            <div className="text-white text-base sm:text-lg font-semibold">{solvedacData.rating}</div>
                        </div>
                        <div className="bg-[#0f0f0f] rounded-xl p-3">
                ...
                <div className="grid grid-cols-2 gap-2 sm:gap-3">
                    <div className="bg-[#0f0f0f] rounded-xl p-3 sm:p-4">
                        ...
                        <div className="text-white text-xl sm:text-2xl font-bold">{solvedacData.solvedCount}</div>
                    </div>
                    <div className="bg-[#0f0f0f] rounded-xl p-3 sm:p-4">
                        <div className="flex items-center gap-2 mb-2">
                            <Zap size={14} className="text-orange-400 sm:w-4 sm:h-4" />
                            <div className="text-gray-400 text-xs font-medium">Max Streak</div>
                        </div>
            ...
            <div className="bg-[#1a1a1a] rounded-2xl p-4 sm:p-6">
                <div className="flex items-center gap-3 mb-4 sm:mb-6">
                    ...
                    <h2 className="text-white text-lg sm:text-xl font-semibold">AtCoder</h2>
                </div>
                ...
                <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3 sm:gap-4 mb-4 sm:mb-5">
                    ...
                    >
                        <span className="text-white font-bold text-base sm:text-lg">{atcoderData.rank}</span>
                    </div>
                    <div className="flex-1 grid grid-cols-3 gap-2 sm:gap-3">
                        <div className="bg-[#0f0f0f] rounded-xl p-2 sm:p-3">
                            <div className="text-gray-400 text-xs font-medium mb-1">Rating</div>
                            <div className="text-white text-sm sm:text-lg font-semibold">{atcoderData.rating}</div>
                        </div>
                        <div className="bg-[#0f0f0f] rounded-xl p-2 sm:p-3">
                            <div className="text-gray-400 text-xs font-medium mb-1">Highest</div>
                            <div className="text-white text-sm sm:text-lg font-semibold">{atcoderData.maxRating}</div>
                        </div>
                        <div className="bg-[#0f0f0f] rounded-xl p-2 sm:p-3">
                            <div className="text-gray-400 text-xs font-medium mb-1">Rank</div>
                ...
                <div className="grid grid-cols-2 gap-2 sm:gap-3 mb-4 sm:mb-5">
                    <div className="bg-[#0f0f0f] rounded-xl p-3 sm:p-4">
                        ...
                        <div className="text-white text-xl sm:text-2xl font-bold">{atcoderData.contests}</div>
                    </div>
                    <div className="bg-[#0f0f0f] rounded-xl p-3 sm:p-4">
                        <div className="flex items-center gap-2 mb-2">
                            <Calendar size={14} className="text-blue-400 sm:w-4 sm:h-4" />
                            <div className="text-gray-400 text-xs font-medium">Last Contest</div>
                        </div>
                        <div className="text-white text-base sm:text-lg font-bold">{atcoderData.lastContestDate}</div>
                    </div>
                </div>

                {/* Recent Contests */}
                <div>
                    <h3 className="text-white text-sm font-semibold mb-2 sm:mb-3">Recent Contests</h3>
                    <div className="space-y-2 max-h-[300px] overflow-y-auto pr-1 sm:pr-2">
                        {[...atcoderData.recentContests].reverse().map((contest, index) => {
                            ...
                            const ratingChangeColor = contest.userRatingChange >= 0 ? "text-green-400" : "text-red-400";
                            const ratingChangeSign = contest.userRatingChange >= 0 ? "+" : "";

                            return (
                                <div key={index} className="bg-[#0f0f0f] rounded-xl p-3">
                                    <div className="text-white text-xs sm:text-sm font-medium mb-2">{contest.name}</div>
                                    <div className="flex flex-col sm:flex-row sm:items-center gap-1 sm:gap-4 text-xs mb-1">
                                        ...
                                        <span className="text-gray-400">
                                            Performance:{" "}
                                            <span className="text-cyan-400 font-semibold">{contest.performance}</span>
                                        </span>
                                        <span className="text-gray-400">
                                            Rating:{" "}
                                            <span className="text-white font-semibold">{contest.userOldRating}</span> →{" "}
                                            <span className="text-white font-semibold">{contest.userNewRating}</span>{" "}
