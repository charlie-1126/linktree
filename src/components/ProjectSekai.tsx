...
export function ProjectSekai() {
    ...
    useEffect(() => {
        ...
        const loadFromCache = () => {
            try {
                const cached = localStorage.getItem(SEKAI_CACHE_KEY);
                if (cached) {
                ...
                console.error("Failed to load cached Sekai data:", e);
            }
            return false;
        };
        ...
        return (
            <div className="bg-[#1a1a1a] rounded-2xl p-4 sm:p-6 animate-pulse">
                ...
                <div className="space-y-2">
                    {[...Array(5)].map((_, i) => (
                        <div key={i} className="bg-[#0f0f0f] rounded-xl p-2 sm:p-3 flex items-center gap-2 sm:gap-3">
    ...
    return (
        <div className="bg-[#1a1a1a] rounded-2xl p-4 sm:p-6">
            <div className="flex items-center gap-3 mb-4 sm:mb-6">
                ...
                <h2 className="text-white text-lg sm:text-xl font-semibold">Project Sekai</h2>
            </div>
            ...
            <div className="grid grid-cols-3 gap-2 sm:gap-3 mb-4 sm:mb-6">
                {sekaiStats.map((stat) => (
                    <div key={stat.label} className="bg-[#0f0f0f] rounded-xl p-2 sm:p-4">
                        <div className="text-gray-400 text-xs font-medium mb-1 sm:mb-2">{stat.label}</div>
                        <div className="text-white text-sm sm:text-lg font-bold">
                            <span className="sm:hidden">{stat.mobileValue || stat.value}</span>
            ...
            {/* Best 39 */}
            <div>
                <h3 className="text-white text-sm font-semibold mb-2 sm:mb-3">Best 39</h3>
                    ...
                    {sekaiData.best39.map((song, index) => (
                        ...
                        >
                            ...
                            <div className="flex-1 min-w-0">
                                <div className="text-white text-xs sm:text-sm font-medium truncate">{song.song}</div>
                            ...
                            <div className="text-right flex flex-col items-end gap-1 flex-shrink-0">
                                ...
                                <div className="text-xs text-gray-400">{Math.round(song.rating)}</div>
                            </div>
                        </div>
                    ))}
                </div>
